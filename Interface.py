...
class Interface:
    ...
    def logIn(self):
        ...
        BotaoAutenticar = customtkinter.CTkButton(janelaLogIn, text="Autenticar", font=("Arial", 15), command=self.verificaSenha)
        BotaoAutenticar.pack(pady=12)
        lembrar = customtkinter.CTkCheckBox(janelaLogIn, text="Lembrar nessa sessão")
        lembrar.pack(pady=8)

        mensagem = customtkinter.CTkLabel(janelaLogIn, text="", font=self.fontePadrao)
        mensagem.pack(pady=15)

    #Método verificar senha
    def verificaSenha(self, mensagem=None):
        if (nomeLog != self.usuario):
            mensagem.configure(text="Usuário inválido",text_color='red')

        elif(senha.get() != self.passe):
    ...
    def novaReserva(self):##<<<<< INTERFACE NOVA JANELA
        janelaEntradaDados = customtkinter.CTkToplevel()
        janelaEntradaDados.title("Nova Reserva")
        fontePadrao = ("Comic Sans MS", "10")
        janelaEntradaDados.geometry("250x505")
        # --------------------------------- CONTAINERS ENTRADA DE DADOS --------------------------------

        titulo = customtkinter.CTkLabel(janelaEntradaDados, text="NOVA RESERVA",font=("Agency FB", 15, "bold"))
        titulo.pack()

        nome = customtkinter.CTkEntry(janelaEntradaDados, placeholder_text="Nome")
        nome.pack(pady=12, padx=10)

        sala = customtkinter.CTkEntry(janelaEntradaDados, placeholder_text="Sala")
        sala.pack(pady=12, padx=10)

        assento = customtkinter.CTkEntry(janelaEntradaDados, placeholder_text="Assento")
        assento.pack(pady=12, padx=10)

        valor = customtkinter.CTkEntry(janelaEntradaDados, placeholder_text="Valor")
        valor.pack(pady=12, padx=10)

        botaoOkEntradaDados = customtkinter.CTkButton(janelaEntradaDados, text="Ok", font=("Comics Sans MS", 10, "bold"),command=Create.Create)
        botaoOkEntradaDados.pack(pady=12)

        novaReservaMsgmLabel = customtkinter.CTkLabel(janelaEntradaDados, text="Campo de confirmação:")
    ...
    def mensagem_edicao(self, botao=None): ##<<<<< INTERFACE MENSAGEM E NOVO PARAMETRO PARA A ESCOLHA EDITAR RESERVA
        ...
        if(txt == "Sala"):
            DescricaoParaAssento = customtkinter.CTkEntry(janelaApagarReserva, placeholder_text="Assento", \
                                                          font=("Arial", 16))
            DescricaoParaAssento.place(x=170, y=80)

        elif (txt == "Assento"):
            DescricaoParaSala = customtkinter.CTkEntry(janelaApagarReserva, placeholder_text="Sala", \
                                                       font=("Arial", 16))
            DescricaoParaSala.place(x=170, y=80)

        elif(txt == "Nome"):
            DescricaoParaSala = customtkinter.CTkEntry(janelaApagarReserva, placeholder_text="Assento",\
                                                       font=("Arial", 16))
            DescricaoParaSala.place(x=170, y=80)
            DescricaoParaAssento = customtkinter.CTkEntry(janelaApagarReserva, placeholder_text="Sala",\
                                                          font=("Arial", 16))
            DescricaoParaAssento.place(x=170, y=110)

        botao.destroy()
        botao2 = customtkinter.CTkButton(janelaApagarReserva, text="EDITAR", command=self.reservaEdit)
        botao2.place(x=100, y=220)

    def mensagens_para_deletar(self): ##<<<<< INTERFACE MENSAGEM DE APAGAR RESERVA
        txt = comboOpcao.get()
        ...
        botao.destroy()
        botao2 = customtkinter.CTkButton(janelaApagarReserva, text="APAGAR", command=self.confirme_delecoes)
        botao2.place(x=100, y=220)
...
botao1 = customtkinter.CTkButton(master=frame, text="Nova RESERVA ", command=Interface.novaReserva)
